...
apiToRow config acc tx = do
  ...
  let txId = getTransactionId acc tx
 ...
 where
  ...
  {-
    ...
    I looked into this, and found out GoCardless recommends using `accountId <> internalTransactionId` as a globally unique transaction ID.
    `internalTransactionId` is generated by GoCardless, and is unique in the scope of a specific account.
    See:
